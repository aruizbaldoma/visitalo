...
- ✅ **Pestaña Analítica en /admin con tracking propio (GDPR-friendly)**:
  - Backend: nuevo router `/api/analytics/{track,heartbeat}` (públicos, gate por consentimiento de cookies en frontend). `track` registra page-views con clasificación de referrer (Google/redes/otros buscadores/directo/interno) y dispositivo (mobile/tablet/desktop), `heartbeat` upserta presencia. País derivado solo de cabeceras del proxy (`cf-ipcountry`/`x-vercel-ip-country`), nunca IP. Nuevas colecciones `analytics_events` + `analytics_presence`.
